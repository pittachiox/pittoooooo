...
class Ball(Widget):
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            ...
            self.ball = Rectangle(source='images/banana-removebg-preview.png', size=(80, 80), pos=(randint(0, Window.width - 80), Window.height - 80))
        self.velocity_y = -200  # ความเร็วของลูกบอลในแนวตั้ง
    ...
    def reset(self):
        # ตั้งค่าลูกบอลใหม่
        self.ball.pos = (randint(0, Window.width - 50), Window.height - 50)
...
class Paddle(Widget):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        with self.canvas:
            self.paddle = Rectangle(size=(150, 20), pos=(Window.width / 2 - 75, 50))
...
class BallCatchGame(Widget):
    ...
    def update_game(self, dt):
        # อัปเดตการเคลื่อนไหวของ paddle และ ball
        ...
        if self.ball.ball.pos[1] < 0:
            print("Game Over!")
            self.ball.reset()

    def check_collision(self):
        # ตรวจสอบการชนระหว่าง paddle และ ball
        px, py = self.paddle.paddle.pos
        bx, by = self.ball.ball.pos
        bw, bh = self.ball.ball.size
...
class BallCatchApp(App):
    def build(self):
        return BallCatchGame()
